...
public class YeniBursiyerGiris extends JFrame {

	private JPanel contentPane;
	private JLabel bursiyerSoyadiLabel;
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					YeniBursiyerGiris frame = new YeniBursiyerGiris();
	...
	public YeniBursiyerGiris() {
		setBackground(Color.WHITE);
		setResizable(false);
		setTitle("Yeni Bursiyer Girişi");
		...
		message1 = new JLabel("Yeni Bursiyer Girişi");
		...
		bursiyerAdi.setToolTipText("Sadece harf geçerlidir. A-Z. Maksimum 30 karakter.");
		bursiyerAdi.setBackground(Color.WHITE);
		...
		bursiyerSoyadiLabel = new JLabel("Bursiyer Soyadı");
		bursiyerSoyadiLabel.setHorizontalAlignment(SwingConstants.CENTER);
		bursiyerSoyadiLabel.setFont(new Font("Tahoma", Font.PLAIN, 14));
		bursiyerSoyadiLabel.setBounds(40, 113, 160, 20);
		contentPane.add(bursiyerSoyadiLabel);
		
		bursiyerSoyadi = new JTextField();
		bursiyerSoyadi.setToolTipText("Sadece harf geçerlidir. A-Z. Maksimum 30 karakter.");
		...
		bursiyerCepTel.setToolTipText("Sadece rakam geçerlidir. 0-9. Boşluk Koymayınız.");
		...
		kaydetButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {		   
				...
				if( !(bursiyerAdi.getText().matches( "[a-z A-ZğĞüÜşŞıİöÖçÇ.,]+" ))  || bursiyerAdi.getText().equals("") || bursiyerAdi.getText().length() > 30) {
					bursiyerAdi.setBackground(Color.PINK);
					error = true;
				}
				else bursiyerAdi.setBackground(Color.WHITE);
					
				if(!(bursiyerSoyadi.getText().matches( "[a-z A-ZğĞüÜşŞıİöÖçÇ.,]+" )) || bursiyerSoyadi.getText().equals("") || bursiyerSoyadi.getText().length() > 30) {
					bursiyerSoyadi.setBackground(Color.PINK);
					error = true;
				}
				else bursiyerSoyadi.setBackground(Color.WHITE);
				...
				if(error) {
					...
					try {
						String BursiyerName = bursiyerAdi.getText();
						String BursiyerSurname = bursiyerSoyadi.getText();
						String OkulTuru = okulTuru.getText();
						String OkulAdi = okulAdi.getText();
						String Sinif = sinif.getText();
						String sehir1 = sehir.getText();
						String IBAN = (iban.getText());
						String BankaAdi = bankaAdi.getText();
						String AileAdresi = aileAdresi.getText();
						String BursiyerMail = bursiyerEmail.getText();
						String BursiyerCepTel = (bursiyerCepTel.getText());
						String EgitimYili = (egitimYili.getText());
						String Aciklama = aciklama.getText();
						Class.forName("com.mysql.cj.jdbc.Driver");
						Connection conn = DriverManager.getConnection(url, username, password);
						Statement statement = conn.createStatement();
						statement.executeUpdate("INSERT INTO  bursiyer (BursiyerAdi, BursiyerSoyadi, OkulTuru, OkulAdi, Sinif, sehir, IBAN, "
								+ "BankaAdi, AileAdresi, BursiyerMail, BursiyerCepTel, EgitimYili,aciklama) VALUES "
								+ "('"+BursiyerName+"','"+ BursiyerSurname + "','"+OkulTuru +"','"+ OkulAdi + "','"+Sinif + "','"+sehir1 +"','"+
								IBAN+ "','"+ BankaAdi + "','"+ AileAdresi+"','"+ BursiyerMail+"','"+ BursiyerCepTel+ "','"+ EgitimYili+"','"+ Aciklama+"')");
						bursiyerAdi.setText("");
						...
						aileAdresi.setText("");
						aciklama.setText("");
						JOptionPane.showMessageDialog(null, "Bursiyer başarıyla sisteme kaydedilmiştir.");
